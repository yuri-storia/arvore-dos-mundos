...
export function friendlyAIError(rawMessage: string): { title: string; hint: string; kind: 'prompt' | 'balance' | 'rate' | 'generic' } {
  ...
  if (
    ...
    return {
      ...
      hint: 'A conexão com a Elixir dos Mundos caiu temporariamente. Aguarde até que a conexão seja reestabelecida — sua quota pessoal de gotas não foi consumida.',
    };
  }
