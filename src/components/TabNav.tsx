...
export const TabNav: React.FC<Props> = ({ activeTab, setActiveTab }) => (
  <nav className="fixed bottom-0 left-0 right-0 z-[150] bg-background/80 backdrop-blur-[12px] border-t border-gold/[0.15] shadow-[0_-4px_20px_rgba(0,0,0,0.5)]">
    <div className="mx-auto max-w-[1060px] flex">
      {TABS.map(tab => (
        ...
        >
          ...
          {activeTab === tab.id && (
            <div className="mx-auto mt-1 w-6 h-[2px] rounded-full bg-gold" />
          )}
        </button>
      ))}
    </div>
