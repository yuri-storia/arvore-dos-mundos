  ...
  return (
    <div className="mx-auto max-w-[1060px] px-3 sm:px-4 py-4">
      <div className="card-glass rounded-lg p-5 sm:p-6">
        ...
        >
          <h2 className="font-cinzel font-bold text-base sm:text-lg text-foreground">
            🌿 Como usar a Árvore dos Mundos
          </h2>
          {expanded ? (
            <ChevronUp className="w-5 h-5 text-blue-light shrink-0" />
          ) : (
            <ChevronDown className="w-5 h-5 text-blue-light shrink-0" />
          )}
        </button>

        {!expanded && (
          <p className="font-merriweather text-text-secondary text-xs sm:text-sm mt-2 leading-relaxed">
