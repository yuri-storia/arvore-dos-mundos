...
import { BookOpen, Leaf, Save, ScrollText, Send, Sparkles, Check } from 'lucide-react';
...
type Bubble =
  | { kind: 'idriel'; id: string; text: string }
...
export const GuidedBuildChat: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    setDraft('');
    setAskMode(false);
    setLog([{ kind: 'idriel', id: 'intro', text: config.intro }]);
  ...
  const pushIdriel = (text: string) =>
    setLog(prev => [...prev, { kind: 'idriel', id: `i-${Date.now()}`, text }]);
  ...
  /** Chips de orientação — visual novel: revelam o ensinamento na própria conversa. */
  const lessonChips = useMemo(() => {
    const chips: { label: string; text: string }[] = config.principles
      .slice(0, 3)
      .map((p, i) => ({ label: p.title ?? `Orientação ${i + 1}`, text: p.body }));
    if (config.caseStudy) chips.push({ label: 'Estudo de caso', text: config.caseStudy });
    return chips;
  }, [config]);
  ...
  return (
    <div className="relative rounded-[26px] border border-gold/20 bg-[radial-gradient(120%_100%_at_0%_0%,hsl(var(--idriel)/0.16),transparent_58%),linear-gradient(180deg,rgba(6,14,26,0.92),rgba(2,7,13,0.96))] shadow-[0_24px_70px_-24px_rgba(0,0,0,0.85)] overflow-hidden">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-gold/50 to-transparent" />

      {/* Idriel — ancorada ao TOPO, sangrando pela esquerda */}
      <div className="pointer-events-none absolute left-0 top-0 z-0 w-[190px] h-[290px] sm:w-[240px] sm:h-[350px] md:w-[320px] md:h-full opacity-95 [mask-image:linear-gradient(to_right,black_60%,transparent_100%)] md:[mask-image:linear-gradient(to_right,black_80%,transparent_100%)]">
        <IdrielStateSprite state={idrielState} heightClass="h-full" className="w-full" objectClass="object-cover object-top" />
      </div>

      <div className="relative z-10 p-4 sm:p-5 md:p-6 md:pl-[300px]">
        {/* Saudação */}
        <div className="mb-4 pl-[86px] sm:pl-[120px] md:pl-0">
          <p className="font-cinzel text-[20px] sm:text-[24px] leading-tight text-foreground">
            Olá, <span className="text-gold-light">Criador!</span>
          </p>
          <p className="font-merriweather text-[12.5px] text-text-dim mt-1">
            {config.name} — o que daremos vida agora?
          </p>
        </div>

        {/* Card de chat */}
        <div className="ml-[86px] sm:ml-[120px] md:ml-0 rounded-2xl border border-gold/15 bg-[rgba(3,9,18,0.82)] backdrop-blur-md overflow-hidden shadow-[0_10px_36px_rgba(0,0,0,0.5)]">
          {/* Barra mínima */}
          <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gold/10">
            ...
            <span className="font-cinzel text-[10px] uppercase tracking-[0.18em] text-gold-light">
              Conversa com Idriel
            </span>
            {questions.length > 0 && (
          ...
          {/* Transcrição — bolhas */}
          <div ref={scrollRef} className="min-h-[240px] max-h-[320px] md:max-h-[400px] overflow-y-auto px-4 py-4 space-y-3.5">
            {log.map(b => b.kind === 'user' ? (
            ...
            {/* Pergunta guiada atual */}
            {!askMode && step && (
              <div className="flex gap-2.5 animate-fadeUp">
                <Leaf className="w-3.5 h-3.5 text-gold-champagne shrink-0 mt-2.5" strokeWidth={1.75} />
                <div className="max-w-[88%] rounded-2xl rounded-bl-md border border-gold/20 bg-gold/[0.06] px-4 py-2.5">
                  <p className="font-cinzel text-[11px] uppercase tracking-wider text-gold-light mb-0.5">{step.label}</p>
                  {step.placeholder && (
            ...
            {aiLoading && (
              <div className="flex items-center gap-1 pl-6 text-text-dim">
                ...
                <span className="ml-2 font-merriweather italic text-xs">Idriel consulta o Codex…</span>
              </div>
            )}
            ...
            {answeredAll && !aiLoading && (
              <div className="flex gap-2.5 animate-fadeUp">
                <Check className="w-3.5 h-3.5 text-gold-light shrink-0 mt-2.5" strokeWidth={2} />
                <p className="max-w-[88%] rounded-2xl rounded-bl-md border border-gold/20 bg-gold/[0.06] px-4 py-2.5 font-merriweather text-[13px] text-text-secondary">
                  {OUTPUT_HINT[config.preferredOutput]} Salve como {outputsLabel}.
                </p>
              </div>
            )}
          </div>
          ...
          {/* Compositor */}
          <div className="border-t border-gold/10 px-4 py-3.5 bg-[rgba(2,7,13,0.72)] space-y-3">
            {/* Chips — orientações (visual novel) + sugestões */}
            <div className="flex gap-2 overflow-x-auto pb-0.5 -mx-0.5 px-0.5">
              {lessonChips.map(c => (
                <button
                  key={c.label}
                  type="button"
                  onClick={() => pushIdriel(c.text)}
                  className="shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-montserrat border border-gold/25 bg-gold/[0.05] text-gold-champagne hover:bg-gold/15 hover:border-gold/50 transition-colors"
                >
                  <BookOpen className="w-3 h-3" strokeWidth={1.75} />
                  {c.label}
                </button>
              ))}
              {!askMode && step?.suggestions?.map(s => (
                <button
                  key={s}
                  type="button"
                  onClick={() => { setAskMode(true); setDraft(s); inputRef.current?.focus(); }}
                  className="shrink-0 px-3 py-1.5 rounded-full text-[11px] font-montserrat border border-blue-bright/25 bg-blue-bright/[0.06] text-text-secondary hover:text-foreground hover:border-blue-bright/50 transition-colors"
                >
                  {s}
                </button>
              ))}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {canUseAI ? (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3.5 py-2 rounded-full text-[10px] font-montserrat font-bold uppercase tracking-wider border transition-colors ${
                    askMode
                      ? 'border-gold-light text-gold-light bg-gold/15'
                      : 'border-gold/30 text-gold-champagne hover:bg-gold/10 hover:border-gold/50'
                  }`}
                >
                  <Sparkles className="w-3 h-3" strokeWidth={2} />
                  {askMode ? 'Voltar a escrever' : 'Pedir ideias · 1 gota'}
                </button>
              ) : upgradeSlot}

              <span className="hidden sm:inline font-montserrat text-[9.5px] text-text-dim/80">
                Vira {outputsLabel}
              </span>
        ...
        {specialSlot && (
          <div className="mt-4 rounded-2xl border border-gold/12 bg-[rgba(3,9,18,0.6)] p-4">{specialSlot}</div>
        )}
      </div>
    </div>
  );
